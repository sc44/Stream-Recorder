#! /usr/bin/env python3
#
#  StreamRecorder - Version: 1.36 - letzte Änderungen: 17.12.2021
#
###############################################################################################################
...
homeVerzeichnis = os.path.expanduser("~")
recVerzeichnis = "/.cache/srecorder/records/"
m3uVerzeichnis = "/.config/srecorder/m3u/"
confVerzeichnis = "/.config/srecorder/"

protDatei = "/.cache/srecorder/protocol.txt"
schedDatei = "/.cache/srecorder/schedule.txt"
confDatei = "/.config/srecorder/srecorder.conf"
keyDatei = "/.config/srecorder/keyboard.conf"
playerDatei = "/.config/srecorder/splayer.conf"
uaDatei = "/.config/srecorder/useragent.conf"
...
"de" : {"Datei":" Datei ","Suchen":" Suchen ","Favoriten":" Favoriten ","Aufnahme":" Aufnahme ","Schedule":" Schedule ",
        ...
        "HauptFenster":"   Begrenztes Hauptfenster für MediaPlayer","Protokoll aus":"   Keine Meldungen ins Protokoll schreiben",
...
"en" : {"Datei":" File ","Suchen":" Search ","Favoriten":" Favorites ","Aufnahme":" Recording ","Schedule":" Schedule ",
        ...
        "HauptFenster":"   Limited startup window for media player","Protokoll aus":"   Don't write any messages in the logfile",
...
Master = tk.Tk()
Master.title("Stream Recorder v1.36")
...
HauptFenster = tk.IntVar()              # Fenstergrösse schmal/breit
...
if not os.path.isdir(homeVerzeichnis + confVerzeichnis):
    os.makedirs(homeVerzeichnis + confVerzeichnis)

if os.path.isfile(homeVerzeichnis + confDatei):                  # wenn Konfigurationsdatei existiert dann laden
    with open(homeVerzeichnis + confDatei, "r") as Datei:
        Puffer.clear() 
    ...
    Vordergrund = Puffer[3][3:10]
    Hintergrund = Puffer[4][3:10]
    FensterVG = Puffer[5][4:11]
    FensterHG = Puffer[6][4:11]
    SizeM = Puffer[7][6:8]
    SizeL = Puffer[8][6:8]
    Gebiet = Puffer[9][9:11]
    HauptFenster.set(int(Puffer[10][7:8]))    
    ProtMeldAus.set(int(Puffer[11][7:8]))    
else:                                                            # sonst neue Konfigurationsdatei erstellen
    if locale.getlocale()[0][0:2] == "de":  Gebiet = "de"
    else:                                   Gebiet = "en"
    HauptFenster.set(0)    
    ProtMeldAus.set(0)    
    with open(homeVerzeichnis + confDatei, "w") as Datei: 
        ...
        Datei.write("window=" + "0\n")
        Datei.write("noprot=" + "0\n")
        Datei.close()
TxT = Woerterbuch[Gebiet]                                        # Zeiger auf aktuelle Sprache

if HauptFenster.get():  Master.geometry("430x875")               # Fenstergrösse beim Start
else:                   Master.geometry("770x875")

if os.path.isfile(homeVerzeichnis + playerDatei):                # wenn Player-Datei existiert dann laden
    with open(homeVerzeichnis + playerDatei, "r") as Datei:
        pListe.clear() 
        for Zeile in Datei:
            pListe.append(Zeile.rstrip())
        Datei.close()
else:                                                            # sonst neue Player-Datei erstellen
    with open(homeVerzeichnis + playerDatei, "w") as Datei:
        for i in range(len(pListe)):
            Datei.write(pListe[i] + "\n")
        Datei.close()
cmdPlayer = pListe[0][24:]                                       # Player-Kommandozeile (default = 1.Zeile)

if os.path.isfile(homeVerzeichnis + uaDatei):                    # wenn User-Agent-Datei existiert dann laden
    with open(homeVerzeichnis + uaDatei, "r") as Datei:
        uaListe.clear() 
        for Zeile in Datei:
            uaListe.append(Zeile.rstrip())
        Datei.close()
else:                                                            # sonst neue User-Agent-Datei erstellen
    with open(homeVerzeichnis + uaDatei, "w") as Datei:
        for i in range(len(uaListe)):
            Datei.write(uaListe[i] + "\n")
        Datei.close()
UserAgent = uaListe[0][30:]                                      # User-Agent-Eintrag (defaut = 1.Zeile)

if os.path.isfile(homeVerzeichnis + keyDatei):                   # wenn Tastaturbelegungsdatei existiert dann laden
    with open(homeVerzeichnis + keyDatei, "r") as Datei:
        keyListe.clear() 
        for Zeile in Datei:
            keyListe.append(Zeile.rstrip())
        Datei.close()
else:                                                            # sonst neue Tastaturbelegungsdatei erstellen
    with open(homeVerzeichnis + keyDatei, "w") as Datei:
        for i in range(len(keyListe)):
            Datei.write(keyListe[i] + "\n")
        Datei.close()

if not os.path.isdir(homeVerzeichnis + m3uVerzeichnis):          # Verzeichnisse erstellen wenn nicht vorhanden
    os.makedirs(homeVerzeichnis + m3uVerzeichnis)
if not os.path.isdir(homeVerzeichnis + recVerzeichnis):
    os.makedirs(homeVerzeichnis + recVerzeichnis)
...
def Datei_Oeffnen(event=None):
    ...
    if not m3uMerker == "":
        m3uMerke2 = m3uDatei
        m3uDatei = fdialog.askopenfilename(initialdir=homeVerzeichnis + m3uVerzeichnis, filetypes=[("Playlists","*.m3u *.m3u8"),("Alle Dateien","*")])
        if m3uDatei == ():  m3uDatei = m3uMerke2
        else:               m3uMerker = m3uMerke2
    else:                                               # wenn m3uMerker == "" (nur einmal beim Programmstart)
        m3uDatei = m3uMerker = homeVerzeichnis + startDatei
        if not os.path.isfile(m3uDatei):                # wenn startDatei nicht existiert
            m3uDatei = m3uMerker = fdialog.askopenfilename(initialdir=homeVerzeichnis + m3uVerzeichnis, filetypes=[("Playlists","*.m3u *.m3u8"),("Alle Dateien","*")])
...
def Datei_Bearbeiten(event=None):
    ...
    def Datei_Speichern(event):
        ...
        Datei = fdialog.asksaveasfile(parent=Fenster, mode="w", initialdir=homeVerzeichnis + m3uVerzeichnis, filetypes = [("Playlists","*.m3u *.m3u8"),("Alle Dateien","*")])
        if Datei:
    ...
    if not os.path.isfile(m3uDatei):
        ...
        Text_Fenster = tk.Text(Fenster, width=150, height=60, pady=10, padx=10, yscrollcommand = Scroll_Vertikal.set, xscrollcommand = Scroll_Horizont.set)
        Text_Fenster.config(foreground=FensterVG, background=FensterHG, font="Monospace 10", wrap="none", undo="TRUE")
...
def Suche_Namen():
    ...
    Button_Speichern.pack(pady=25, ipadx=4)
...
def Favoriten_Anzeigen(event=None):
    ...
    if not os.path.isfile(homeVerzeichnis + m3uVerzeichnis + "favoriten.m3u"):
        message.showwarning("Stream Recorder", "\n" + m3uVerzeichnis + "favoriten.m3u" + TxT["nicht gefunden"])
    else:
        m3uMerker = m3uDatei
        m3uDatei = homeVerzeichnis + m3uVerzeichnis + "favoriten.m3u"
        with open(m3uDatei, "r") as Datei:
            Puffer.clear()
            for Zeile in Datei:
                    Puffer.append(Zeile)
...
def Favoriten_Hinzufuegen(event=None):
    ...
    if Listen_Box.curselection():
        ...
        FavName = homeVerzeichnis + m3uVerzeichnis + "favoriten.m3u"
...
def Favoriten_Entfernen():
    ...
    if m3uDatei == homeVerzeichnis + m3uVerzeichnis + "favoriten.m3u":
        Nr = Listen_Box.curselection()[0]           # Index des markierten Senders
...
def Player_Auswaehlen(event=None):
    ...
    Fenster.geometry("215x370")
    Fenster.wm_attributes("-topmost", True)
    Fenster.wait_visibility()
    Fenster.grab_set()
    Player_Liste = tk.Listbox(Fenster, width=150, height=60, selectborderwidth=2)
...
def User_Agent_Aendern(event=None):
    ...
    Fenster.geometry("250x350")
    Fenster.wm_attributes("-topmost", True)
    Fenster.wait_visibility()
    Fenster.grab_set()
    UserAgent_Liste = tk.Listbox(Fenster, width=150, height=60, selectborderwidth=2)
...
def Stream_Aufnehmen(event=None):
    ...
    def protDatei_Schreiben(text, name):
        ...
        if not ProtMeldAus.get():  
            with open(homeVerzeichnis + protDatei, "a") as Datei:
    ...
    if Listen_Box.curselection():
        ...
        try:                                             # URL prüfen
            ...
            for n in range(1, 1000):                     # Namen der Aufnahme festlegen (..._001 bis ..._999)
                ...
                if not os.path.isfile(homeVerzeichnis + recVerzeichnis + Dateiname):     # wenn Dateiname nicht existiert dann übernehmen
                    break
            #subprocess.Popen('ffmpeg -i "' + URL[Nr] + '" -c:v copy -c:a copy "' + homeVerzeichnis + recVerzeichnis + Dateiname + '"', shell=True)
            subprocess.Popen('ffmpeg -i "' + URL[Nr] + '" -c:v copy -c:a copy "' + homeVerzeichnis + recVerzeichnis + Dateiname + '" 2> /dev/null', shell=True)
            time.sleep(0.5)
...
def Schedule_Starten():
    ...
    if os.path.isfile(homeVerzeichnis + schedDatei):
        with open(homeVerzeichnis + schedDatei, "r") as Datei:          # Schedule-Datei in Puffer laden
            for Zeile in Datei:
        ...
        if Startzeit == Aktuelle and sPuffer[i][w] == "x":                # wenn Uhrzeit und Wochentag stimmen
            try:                                                          # URL prüfen
                ...
                for n in range(1, 1000):                                  # Namen der Aufnahme festlegen (..._001 bis ..._999)
                    ...
                    if not os.path.isfile(homeVerzeichnis + recVerzeichnis + Dateiname):
                        break
                #subprocess.Popen('ffmpeg -i "' + sPuffer[i+1].rstrip() + '" -c:v copy -c:a copy "' + homeVerzeichnis + recVerzeichnis + Dateiname + '"', shell=True)
                subprocess.Popen('ffmpeg -i "' + sPuffer[i+1].rstrip() + '" -c:v copy -c:a copy "' + homeVerzeichnis + recVerzeichnis + Dateiname + '" 2> /dev/null', shell=True)
                time.sleep(0.5)
                pid = int(subprocess.check_output(["pidof", "-s", "ffmpeg"]))
                recPID.append(pid)                                        # Aufnahme: PID speichern
                recStart.append(time.strftime("%H%M"))                    # Aufnahme: Startzeit speichern
            ...
            if sPuffer[i][20] == "*":
                ...
                with open(homeVerzeichnis + schedDatei, "w") as Datei:  # schedule.txt neu schreiben
...
def Protokoll_Anzeigen(event=None):
    ...
    if not os.path.isfile(homeVerzeichnis + protDatei):
        message.showwarning("Stream Recorder", "\n" + protDatei + TxT["nicht gefunden"])
    else:
        Fenster = tk.Toplevel(Master)
        Fenster.title(TxT["Protokoll"])
        Fenster.geometry("650x540")
        Fenster.wm_attributes("-topmost", True)
        Fenster.grab_set()
        Scroll_Balken = tk.Scrollbar(Fenster, width=14)
        Text_Fenster = tk.Text(Fenster, width=150, height=60, pady=10, padx=10, yscrollcommand=Scroll_Balken.set)
        ...
        Text_Fenster.pack(fill="both", padx=2, pady=2)
        with open(homeVerzeichnis + protDatei, "r") as Datei:
...
def Schedule_Anzeigen(event=None):
    ...
    if not os.path.isfile(homeVerzeichnis + schedDatei):
        message.showwarning("Stream Recorder", "\n" + schedDatei + TxT["nicht gefunden"])
    else:
        Fenster = tk.Toplevel(Master)
        Fenster.title("Schedule")
        Fenster.geometry("510x440")
        Fenster.wm_attributes("-topmost", True)
        Fenster.grab_set()
        ...
        with open(homeVerzeichnis + schedDatei, "r") as Datei:
            Text_Fenster.configure(state="normal")
            Text_Fenster.delete("1.0", tk.END)
            for Zeile in Datei:
...
def Schedule_Hinzufuegen(event=None):
    ...
    def Eintrag_Speichern(event=None):
        ...
        with open(homeVerzeichnis + schedDatei, "a") as Datei:
...
#----------------------------------------------------
    ...
    ProgName.grid(row=0, column=2, padx=0, pady=20, columnspan=12)
    tk.Label(Fenster).grid(row=1, column=1, padx=10)
    TextMo.grid(row=1, column=2, padx=2, pady=4)
    TextDi.grid(row=1, column=3, padx=2, pady=4)
    TextMi.grid(row=1, column=4, padx=2, pady=4)
    TextDo.grid(row=1, column=5, padx=2, pady=4)
    TextFr.grid(row=1, column=6, padx=2, pady=4)
    TextSa.grid(row=1, column=7, padx=2, pady=4)
    TextSo.grid(row=1, column=8, padx=2, pady=4)
    ...
    BoxMo.grid(row=2, column=2, padx=1, pady=4)
    BoxDi.grid(row=2, column=3, padx=1, pady=4)
    BoxMi.grid(row=2, column=4, padx=1, pady=4)
    BoxDo.grid(row=2, column=5, padx=1, pady=4)
    BoxFr.grid(row=2, column=6, padx=1, pady=4)
    BoxSa.grid(row=2, column=7, padx=1, pady=4)
    BoxSo.grid(row=2, column=8, padx=1, pady=4)
...
def Schedule_Bearbeiten(event=None):
    ...
    def Schedule_Speichern(event):
        ...
        with open(homeVerzeichnis + schedDatei, "w") as Datei:
            Datei.write(Text_Fenster.get("1.0", tk.END + "-1c"))       # ohne letztes LF !!
            Datei.close()
        Fenster.destroy()   
...
def Einstellungen(event=None):
    ...
    def Fensterfarbschema():
        ...
        Fenster2.title("")
        Fenster2.geometry("250x258")
        Fenster2.wm_attributes("-topmost", True)
        Fenster2.grab_set()

        Farben_Liste = tk.Listbox(Fenster2, width=22, font="Helvetica 13")
        Farben_Liste.pack(side="left", fill="y", padx=13, pady=13)
        for name in Farben:
    ...
    def Einstellungen_Speichern():
        ...
        if not os.path.isdir(homeVerzeichnis + m3uVerzeichnis):
            os.makedirs(homeVerzeichnis + m3uVerzeichnis)
        if not os.path.isdir(homeVerzeichnis + recVerzeichnis):
            os.makedirs(homeVerzeichnis + recVerzeichnis)

        if HauptFenster.get():  Master.geometry("430x875")
        else:                   Master.geometry("770x875")
        Master.eval("tk::PlaceWindow . center")
        Statusleiste_Anzeigen("")

        m3uVerzeichnis = m3uEingabe.get()
        recVerzeichnis = recEingabe.get()
        ...
        with open(homeVerzeichnis + confDatei, "w") as Datei: 
            ...
            Datei.write("size2=" + SizeL + "\n")
            Datei.write("language=" + Gebiet + "\n")
            Datei.write("window=" + str(HauptFenster.get()) + "\n")
...
#-----------------------------------------------------
    ...
    Check_StartBild = tk.Checkbutton(Fenster, bd=2, text=TxT["HauptFenster"], font="Helvetica 11", variable=HauptFenster)
...
def Hilfe_Tastatur(event=None):
    ...
    if not os.path.isfile(homeVerzeichnis + keyDatei):
        message.showwarning("Stream Recorder", "\n" + keyDatei + " nicht gefunden.  ")
    else:
        Fenster = tk.Toplevel(Master)
        Fenster.title(TxT["Tastatur"])
        Fenster.geometry("370x620")
        Fenster.wm_attributes("-topmost", True)
        Fenster.grab_set()
        Text_Fenster = tk.Text(Fenster, width=150, height=60, pady=10, padx=10)
        Text_Fenster.config(foreground=FensterVG, background=FensterHG, font="Monospace 10", wrap="none")
...
def Programm_Beenden(event=None):
    ...
    if StatusAufnahmen == 0:
        ...
        if message.askokcancel("Stream Recorder", "\nSoll die Aufnahme wirklich beendet werden?  "):
            subprocess.Popen('killall ffmpeg', shell=True)
            if not ProtMeldAus.get():  
                with open(homeVerzeichnis + protDatei, "a") as Datei:
